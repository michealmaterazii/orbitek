...
const ADMIN_IDS = ["13430808-227c-4bc5-a2d2-618ab23c5710"];
...
export default function AdminPanel() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#010108] flex items-center justify-center">
      ...
      <div style={{ fontFamily: "'Orbitron', sans-serif", color: '#ff4444', fontSize: '12px', letterSpacing: '0.3em' }}>LOADING ADMIN...</div>
    </div>
  );
  ...
  const totalRevenue = orders.reduce((sum, o) => sum + parseFloat(o.price || 0), 0).toFixed(2);
  ...
  return (
    <main className="min-h-screen bg-[#010108] text-white" style={{ fontFamily: "'Rajdhani', sans-serif" }}>
      ...
      {sidebarOpen && (
        <div className="fixed md:hidden" style={{ top: 0, left: '240px', right: 0, bottom: 0, zIndex: 40, background: 'rgba(0,0,0,0.6)' }} onClick={() => setSidebarOpen(false)} />
      )}
        ...
        <aside className={`fixed md:relative top-0 left-0 h-full z-50 flex flex-col transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}
          ...
          <div className="flex items-center gap-3 px-4 py-5" style={{ borderBottom: '1px solid rgba(255,0,0,0.1)' }}>
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: 'rgba(255,0,0,0.6)', fontSize: '9px', letterSpacing: '0.2em' }}>ADMIN PANEL</p>
            </div>
          ...
          <a href="/dashboard" className="flex items-center gap-3 px-4 py-3 transition-all hover:bg-white/5"
            style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            <span className="text-xs tracking-widest" style={{ fontFamily: "'Orbitron', sans-serif", color: 'rgba(255,255,255,0.3)', letterSpacing: '0.15em' }}>← USER PANEL</span>
          </a>
          ...
          <button onClick={handleLogout}
            ...
            <LogOut size={17} style={{ color: 'rgba(255,0,0,0.4)' }} />
            ...
            {active === "Overview" && (
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                  {[
                    { label: "TOTAL ORDERS", value: orders.length, color: '#00ffff' },
                    { label: "PENDING ORDERS", value: pendingOrders, color: '#ffcc00' },
                    { label: "TOTAL REVENUE", value: `$${totalRevenue}`, color: '#00ff88' },
                  ].map((stat, i) => (
            ...
            {active === "Users" && (
              <div>
                ...
                <div className="p-8 text-center" style={{ border: '1px solid rgba(255,255,255,0.05)', background: 'rgba(255,255,255,0.02)' }}>
                  ...
                  <p className="text-xs tracking-widest mb-2" style={{ fontFamily: "'Orbitron', sans-serif", color: 'rgba(255,255,255,0.2)', letterSpacing: '0.2em' }}>VIEW USERS IN SUPABASE</p>
                  <a href="https://supabase.com/dashboard/project/hqotdvjocztcxfdtkgdw/auth/users" target="_blank"
                    className="inline-block mt-4 text-xs px-6 py-3 tracking-widest transition-all hover:opacity-80"
