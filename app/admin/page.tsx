...
import { Users, ShoppingBag, DollarSign, RefreshCw, LogOut, Wifi, Menu, X } from "lucide-react";
...
export default function AdminPanel() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#010108] flex items-center justify-center">
      ...
      <div style={{ fontFamily: "'Orbitron', sans-serif", color: '#00ffff', fontSize: '12px', letterSpacing: '0.3em' }}>LOADING...</div>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-[#010108] text-white" style={{ fontFamily: "'Rajdhani', sans-serif" }}>
      ...
      <div className="flex h-screen overflow-hidden relative z-10">
        ...
        {/* Sidebar */}
        <aside className={`fixed md:relative top-0 left-0 h-full z-50 flex flex-col transition-transform duration-300 w-60 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}
          ...
          <div className="px-4 py-3 mx-4 mb-4 text-xs" style={{ border: '1px solid rgba(255,0,0,0.15)', background: 'rgba(255,0,0,0.05)' }}>
            <p style={{ color: 'rgba(255,100,100,0.7)', fontFamily: "'Rajdhani', sans-serif", fontSize: '10px', letterSpacing: '0.1em' }}>LOGGED IN AS</p>
            <p className="truncate" style={{ color: 'rgba(255,255,255,0.6)', fontSize: '11px' }}>{user?.email}</p>
          </div>
                    ...
                    <div key={order.id} className="p-4" style={{ border: '1px solid rgba(255,255,255,0.05)', background: 'rgba(255,255,255,0.02)' }}>
                      <div className="flex items-start justify-between gap-4 mb-3">
                        ...
                        <div className="shrink-0 text-right">
                          <span className="text-xs px-2 py-1 block mb-2" style={{
                            fontFamily: "'Orbitron', sans-serif", fontSize: '9px',
                            border: `1px solid ${order.status === 'completed' ? 'rgba(0,255,100,0.4)' : 'rgba(255,200,0,0.4)'}`,
                            color: order.status === 'completed' ? 'rgba(0,255,100,0.8)' : 'rgba(255,200,0,0.8)',
                          }}>{order.status?.toUpperCase()}</span>
            ...
            {active === "Users" && (
              <div>
                ...
                <div className="p-8 text-center" style={{ border: '1px solid rgba(255,255,255,0.05)', background: 'rgba(255,255,255,0.02)' }}>
                  ...
                  <p className="text-xs" style={{ fontFamily: "'Rajdhani', sans-serif", color: 'rgba(255,255,255,0.3)' }}>
